...
for robot in robots:
    while True:
        try:
            lat = float(input(f"Enter the latitude for robot {robots.index(robot)+1}: "))
            if -90 <= lat <= 90:
                robot.latitude = lat
                break
            else:
    ...
    while True:
        try:
            lng = float(input(f"Enter the longitude for robot {robots.index(robot)+1}: "))
            if -180 <= lng <= 180:
                robot.longitude = lng
                break
            else:
    ...
    while True:
        disabled = input(f"Is robot {robots.index(robot)+1} disabled? (y/n): ")
        if disabled.lower() == "y":
...
for robot in robots:
    new_speed = int(input(f"Enter new motor speed for robot {robots.index(robot)+1}: "))
    new_direction = int(input(f"Enter new direction for robot {robots.index(robot)+1}: "))
    new_range = int(input(f"Enter new sensor range for robot {robots.index(robot)+1}: "))
